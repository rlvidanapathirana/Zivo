...
export default function VideoPlayer({ videoId, title, channelTitle, thumbnail, isMini }) {
  ...
  return (
    <div id={`zivo-player-host-${videoId}`} className="w-full h-full">
      ...
      >
      {/* Top Floating Shields, PiP & Status Overlay (Auto-Hiding & Mobile Responsive) */}
      {!isMini && (
        <>
          <div className={`absolute top-2 right-2 sm:top-3 sm:right-3 z-40 flex flex-wrap items-center justify-end gap-1.5 sm:gap-2 max-w-[calc(100%-16px)] transition-opacity duration-300 ${showOverlay ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            {/* Picture in Picture Button */}
            <button
              onClick={handleTogglePiP}
              title="Picture-in-Picture Floating Mode"
              className="flex items-center gap-1 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full text-[11px] sm:text-xs font-bold bg-black/80 backdrop-blur-md text-white/90 hover:bg-black/90 border border-white/20 transition-all shadow-md active:scale-95"
            >
              <PictureInPicture2 size={13} className="text-purple-400" />
              <span>PiP Mode</span>
            </button>
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full text-[11px] sm:text-xs font-bold backdrop-blur-md transition-all shadow-md ${
                audioOnlyMode
                  ? 'bg-purple-600 text-white shadow-purple-600/50 ring-2 ring-purple-400'
                  : 'bg-black/80 text-white/90 hover:bg-black/90 border border-white/20'
              }`}
            >
              <Headphones size={13} />
              <span>{audioOnlyMode ? 'Audio' : 'Screen-Off'}</span>
            </button>
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full text-[11px] sm:text-xs font-bold bg-purple-600/90 backdrop-blur-md text-white hover:bg-purple-500 border border-purple-400/40 transition-all shadow-md active:scale-95"
            >
              {copiedZivoLink ? <Check size={13} className="text-emerald-300" /> : <Share2 size={13} className="text-white" />}
              <span>{copiedZivoLink ? 'Copied!' : 'Share Link'}</span>
            </button>
            ...
            <button
              ...
              className="hidden xs:flex items-center gap-1 px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-full bg-black/80 backdrop-blur-md text-emerald-400 border border-emerald-500/30 text-[10px] sm:text-xs font-semibold hover:bg-black/90 transition-colors shadow-md"
            >
              <ShieldCheck size={13} className="text-emerald-400" />
              <span className="text-[10px] text-white">Shield</span>
            </button>
          </div>

          {/* Bottom-Left Dedicated Zivo Share Button Overlay (Over YouTube iframe link icon position) */}
          <div className={`absolute bottom-3 left-2 sm:bottom-12 sm:left-3 z-40 transition-opacity duration-300 ${showOverlay ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <button
              onClick={handleCopyZivoLink}
              title="Copy Zivo Video Link (Site URL)"
              className="flex items-center gap-1 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full text-[11px] sm:text-xs font-extrabold bg-black/85 text-purple-300 hover:text-white hover:bg-purple-600 backdrop-blur-md shadow-xl border border-purple-500/40 transition-all active:scale-95"
            >
              {copiedZivoLink ? <Check size={13} className="text-emerald-400" /> : <Share2 size={13} className="text-purple-400 hover:text-white" />}
